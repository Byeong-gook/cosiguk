...
public class b_IpstoreService {
    ...
    public void ipAdd(Long id, String ip) {
        ...
        b_Ipstore b_ipstore = new b_Ipstore();
        b_ipstore.setIp(ip);
        b_ipstore.setBoard(board);
        em.persist(b_ipstore);
    }

    //board_id와 클라이언트 ip를 통해 게시글에 저장되있는 ip 데이터 가져오기
    public String IpSelectQuery(long boardId, String ip) throws SQLException {
        String jpql = "SELECT i FROM b_ipstore i where ip = :ipAddress and board_id = :boardAddress";
        TypedQuery<b_Ipstore> query = em.createQuery(jpql, b_Ipstore.class);
        query.setParameter("ipAddress", ip);
        query.setParameter("boardAddress", boardId);


        List<b_Ipstore> ipList = query.getResultList();

        for (b_Ipstore b_ipstore : ipList) {
            String checkIp = b_ipstore.getIp();
